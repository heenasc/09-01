...
function View() {
  ...
  console.log("StartD,EndD",StartD,EndD);
   // Call fetchproject and wait for it to complete
   const projectPromises = projectKey.map((Key) => fetchproject(Key));
   const allProjectsRoles = await Promise.all(projectPromises);

    // Flatten the arrays of roles
    const roles = allProjectsRoles.flat();

    console.log("Roles for all projects:", roles);
...
const filterInactiveUsers = async (accountIds) => {
 ...
 /* const fetchData = async (role,project,StartDate,EndDate) => {
    ...
    const fetchData = async (roles, project, startDate, endDate, selectedDateUnit) => {
  try {
  console.log("fetchdata after fetchAndSetInterval");
    // Iterate through each project
    const projectResponses = await Promise.all(
      project.map(async (project) => {
        // Iterate through each role for the current project
        const roleResponses = await Promise.all(
          roles.map(async (role) => {
            ...
            // Other code...
            return responseJson; // Return the role response for further processing
          })
        );

        // Combine role responses for the current project
        return { project, roles: roleResponses };
      })
    );

    // Process projectResponses as needed...

    // Extract data after the asynchronous operations are complete
    const accountIds = projectResponses.flatMap(({ roles }) =>
      roles.flatMap((roleResponse) =>
        roleResponse.actors.map((actor) => actor.actorUser.accountId)
      )
    );

    console.log("projectResponses accountIds", accountIds);
    const activeAccountIds = await filterInactiveUsers(accountIds);
    ...
    //const intervalId = setInterval(() => fetchData(roles, projects, startDate, endDate, selectedDateUnit), getMillisecondsForInterval(refreshInterval));
  ...
  //  return () => clearInterval(intervalId);
    ...
    //below code is for multiple roles
   /* const fetchData = async (roles, projects, startDate, endDate, selectedDateUnit) => {
  try {
    // Iterate through each project
    const projectResponses = await Promise.all(
    ...
    console.log("projectResponses", projectResponses);
    const accountIds = projectResponses.flatMap(({ roles }) =>
      roles.flatMap((roleResponse) =>
        roleResponse.actors.map((actor) => actor.actorUser.accountId)
      )
    ...
    let query1 = await JQLFucntion(accountIds, startDate, endDate, projects, selectedDateUnit);
    setQuery(query1);

    const userWorkLogs = await Promise.all(
      activeAccountIds.map((accountId) =>
        fetchIssues(accountId, StartDate, EndDate, projects, selectedDateUnit)
      )
    );
    ...
    //below code is working fine for multiple projects
    /*const fetchData = async (role, projects, StartDate, EndDate, selectedDateUnit) => {
  try {
    // Iterate through each project
    const projectResponses = await Promise.all(
      projects.map(async (project) => {
        ...
        // Other code...
        return responseJson; // Return the project response for further processing
      })
    );

    // Process project responses as needed...
    // For example, you can iterate through projectResponses and extract data
    ...
    //const accountIds = projectResponses.actors.map((actor) => actor.actorUser.accountId);
    console.log("projectResponses", projectResponses);
    const accountIds = projectResponses.flatMap((response) =>
      response.actors.map((actor) => actor.actorUser.accountId)
    );
    console.log("projectResponses accountIds", accountIds);
    const activeAccountIds = await filterInactiveUsers(accountIds);
    ...
    let Query1 = await JQLFucntion(activeAccountIds, StartDate, EndDate, projects, selectedDateUnit);
    setQuery(Query1);
    console.log("accountIds from view", accountIds);
    ...
    //below code working fine for single select project
  ...
  try {
   ...
   //Query = `project = "${project}" AND worklogAuthor in (${activeAccountIds}) AND worklogDate >= "${StartDate}" AND worklogDate <= "${EndDate}"`;
   if (selectedDateUnit === 'Last Week') {
    Query = `project in (${project}) AND worklogAuthor in (${activeAccountIds}) AND worklogDate >= startOfWeek(-1) AND worklogDate <= startOfWeek()`;
    console.log("Query",Query);
    }
    else if (selectedDateUnit === 'Last Month') {
    Query = `project in (${project}) AND worklogAuthor in (${activeAccountIds}) AND worklogDate >= startOfMonth(-1) AND worklogDate < startOfMonth()`;
    console.log("Query",Query);
    }
    else if (selectedDateUnit === 'Last 3 Months'){
    Query = `project in (${project}) AND worklogAuthor in (${activeAccountIds}) AND worklogDate >= startOfMonth(-3) AND worklogDate < startOfMonth()`;
    console.log("Query",Query);
    }
    else{
    Query = `project in (${project}) AND worklogAuthor in (${activeAccountIds}) AND worklogDate >= "${StartDate}" AND worklogDate <= "${EndDate}"`;
    console.log("Query",Query);
    }
   console.log("inside JQLFucntion Query",Query);
   return Query; // Return the Query value
  ...
  async function fetchIssues(accountIds,start,end,project,selectedDateUnit){
    ...
    let maxResults = 50;
    if (selectedDateUnit === 'Last Week') {
    jqlQuery = `project in (${project}) AND worklogAuthor in (${accountIds}) AND worklogDate >= startOfWeek(-1) AND worklogDate <= startOfWeek()`;
    console.log("jqlQuery",jqlQuery);
    }
    else if (selectedDateUnit === 'Last Month') {
    jqlQuery = `project in (${project}) AND worklogAuthor in (${accountIds}) AND worklogDate >= startOfMonth(-1) AND worklogDate < startOfMonth()`;
    console.log("jqlQuery",jqlQuery);
    }
    else if (selectedDateUnit === 'Last 3 Months'){
    jqlQuery = `project in (${project}) AND worklogAuthor in (${accountIds}) AND worklogDate >= startOfMonth(-3) AND worklogDate < startOfMonth()`;
    console.log("jqlQuery",jqlQuery);
    }
    else{
    jqlQuery = `project in (${project}) AND worklogAuthor in (${accountIds}) AND worklogDate >= "${start}" AND worklogDate <= "${end}"`;
    console.log("jqlQuery",jqlQuery);
    }
    ...
    while(loop){
      ...
      let responseJson = await response.json();
      console.log("fetchissue responseJson",responseJson);
      const issueArray = responseJson.issues.map(obj => obj.key);
      issues.push(...issueArray);
      console.log("fetchissue responseJson",responseJson);
      // console.log(responseJson.issues.length);
...
async function getContext() {
  try {
    ...
    console.log(contexts.extension.gadgetConfiguration.context.selectedDateUnit);
    let role = contexts.extension.gadgetConfiguration.context.role;
    console.log("role value",contexts.extension.gadgetConfiguration.context.role);
    let project = contexts.extension.gadgetConfiguration.context.project;
    setProjectsname(project);
    let StartDate = contexts.extension.gadgetConfiguration.context.startDate;
    let EndDate = contexts.extension.gadgetConfiguration.context.endDate;
    ...
    let mintime;
    setRefreshInterval(selectedRefreshInterval);
    ...
    mintime=60;
    setmintime(mintime);
    }else if(selectedRefreshInterval == 'Every 3 hour'){
    mintime=180;
    setmintime(mintime);
    }else{
    mintime=0;
    ...
    console.log("role",role,project, StartDate, EndDate);
    let newmaxHours=40;
     //fetchData(role,project,StartDate,EndDate); // Call the async function here
    ...
    if (role.includes('All')) {
      ...
      if (selectedDateUnit === 'Last Week') {
       ...
       console.log("maxHours",maxHours);
        ...
        if(mintime===0){
        console.log("come inside mintime if");
        fetchAllRoles(project, StartDateRangeAll, EndDateRangeAll,selectedDateUnit);
    	}else{
    	console.log("come outside inside mintime if");
  ...
  setRemainingTime(prevTime => {
    ...
    return () => {
      ...
      if (selectedDateUnit === 'Last Week') {
        ...
        StartDateRange = lastWeekStartDate.toISOString().split('T')[0]; // Extract date portion
        EndDateRange = currentDate.toISOString().split('T')[0]; // Extract date portion
        console.log("all Not Calling Last Week StartDateRange, EndDateRange",role, project,StartDateRange, EndDateRange);
        ...
        const fetchAndSetInterval = () => {
    console.log("fetchAndSetInterval", fetchAndSetInterval);
    console.log("fetchAndSetInterval project", project);
    fetchData(role, project, StartDateRange, EndDateRange, selectedDateUnit);
    setLastRefreshTime(new Date()); // Update last refresh time
};

// Call fetchAndSetInterval initially
fetchAndSetInterval();

let intervalId, timerId;

if (mintime > 0) {
    // Schedule periodic calls based on the refresh interval
    const selectedRefreshInterval = mintime * 60 * 1000;
    ...
    timerId = setInterval(() => {
        setRemainingTime(prevTime => {
            const newTime = Math.max(prevTime - 1, 0);
            if (newTime === 0) {
                // Reset the timer and fetch fresh data
                fetchAndSetInterval();
                return mintime * 60;
            } else {
                return newTime;
...
/*const formatElapsedTime = (current, previous) => {
    const millisecondsPerMinute = 60 * 1000;
    const elapsedMinutes = (current - previous) / millisecondsPerMinute;
    return elapsedMinutes < 1 ? 'Just Now' : `${Math.round(elapsedMinutes)} min ago`;
};*/

const formatElapsedTime = (current, previous) => {
    const millisecondsPerMinute = 60 * 1000;
    const elapsedMinutes = Math.floor((current - previous) / millisecondsPerMinute);
    if (elapsedMinutes < 1) {
        return 'Just Now';
...
<div>
  <div> 
    <p>The selected project is: <strong>{projectname.join(', ')}</strong></p>
    {loggedWork && (
      <table>
        <thead>
          <tr>
